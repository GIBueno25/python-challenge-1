        ...
        if int(menu_category) in menu_items.keys():
            ...
            if menu_selection.isdigit():
                ...
                # 4. Check if the menu selection is in the menu items
                    ...
                    quantity_input = input(f"How many {selected_item_name}s would you like to order? (Default is 1 if input is invalid): ")

                    # Check if the quantity is a number, default to 1 if not
                    if not quantity_input.isdigit():
                        print(f"Invalid input. Quantity will be set to 1.")
                        quantity = 1
                    else:
...
print(f"{'Qty':<5} {'Item Name':<25} {'Price per Unit':<15} {'Total':>8}")
print("-" * 58)
...
for item in order:
    ...
    # Printing each item with proper alignment
    print(f"{item['Quantity']:<5} {item['Item name']:<25} ${item['Price']:<15.2f} ${item_total:>7.2f}")

print("-" * 58)
print(f"{'TOTAL':<48} ${total_cost:>7.2f}")
print("=" * 58)
